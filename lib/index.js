...
import { getUnitDocs, saveUnitDocs, getUnitDicts, saveUnitDicts, searchDict, updateColor } from './pouchdb.js'
...
export async function fanki(config) {
  ...
  let unitdocs = await getUnitDocs(config.unit)
  // let unitdocs = []
  if (!unitdocs.length) {
    let str = getFile(config)
    if (!str) return
    let docs = parseDocs(str)
    await saveUnitDocs(config.unit, docs)
    unitdocs = docs
  }

  let unitdicts = await getUnitDicts(config.unit)
  // let unitdicts = []
  if (!unitdicts.length) {
    let dicts = collectDicts(config)
    log('_BIG DICTS', dicts.length)
    await saveUnitDicts(config.unit, dicts)
    unitdicts = dicts
  }
...
/*
  цвет: либо читать эту карту, а сейчас я имею cards,
  либо сохранить эту карту, и перепрочитать cards
  при сл. загрузке будет новый массив cards
...
function startFanki(unitname, cards) {
  ...
  let card = {
    ...
    changeColor(dir) {
      ...
      else if (doc.color == 1) rl.setPrompt(chalk.yellow(circle))
      else if (doc.color == 2) rl.setPrompt(chalk.red(circle))
      updateColor(unitname, doc)
      this.show()
    }
  }
...
function getFile(config) {
  try {
    return fse.readFileSync(config.unit).toString().trim()
  } catch (err) {
    log('_ERR FILE CARDS')
  }
  return ''
...
function collectDicts(config) {
  let dirs = config.unit.split('/') //.slice(0,-1)
